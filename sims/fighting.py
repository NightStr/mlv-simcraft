...
class FightingSimConfig(NamedTuple):
    ...
    player_attack_interval: decimal.Decimal

    enemy_health: int
    enemy_damage_min: int
    enemy_damage_max: int
    enemy_hit_chance: float
    enemy_attack_interval: decimal.Decimal
...
def simulate_battle(config: FightingSimConfig) -> FightingSimResult:
    ...
    while player_current_health > 0 and time < 5 * 60 * 60:
        ...
        if enemy_current_health <= 0:
            respawn_time_counter += decimal.Decimal('0.1')
            if respawn_time_counter == RESPAWN_TIME:
                enemy_current_health = config.enemy_health
                enemy_killed += 1
                respawn_time_counter = 0
        # Enemy attacks
        else:
            if time % config.enemy_attack_interval == 0:
...
if __name__ == "__main__":
    ...
    config = FightingSimConfig(
        ...
        player_attack_interval=decimal.Decimal('3.0'),
        enemy_health=300,
        enemy_damage_min=0,
        enemy_damage_max=116,
        enemy_hit_chance=0.1,
        enemy_attack_interval=decimal.Decimal('2.4'),
    )
    results = []
